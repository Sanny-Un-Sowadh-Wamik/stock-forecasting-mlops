"""Premium Streamlit dashboard for the ASX forecaster.

Self-contained: loads the committed model directly (no API needed). Animated hero,
branded charts, a forecast uncertainty cone, model comparison and an out-of-sample
backtest.
"""
...
try:
    ...
    st.error("Model artifacts not found. Run `python scripts/train.py`, then reload.")
    st.stop()
...
theme.hero(
    "📈 ASX Stock-Price Forecaster",
    "Walk-forward-validated XGBoost across 10 ASX large-caps · ingestion → features → MLflow → serving. "
...
with tab_fc:
    ...
    if fclose:
        ...
        sigma = float(rets.std()) if len(rets) > 5 else 0.02
        steps = np.arange(1, len(fclose) + 1)
        upper = np.array(fclose) * np.exp(1.28 * sigma * np.sqrt(steps))
        lower = np.array(fclose) * np.exp(-1.28 * sigma * np.sqrt(steps))
        fig.add_trace(go.Scatter(x=fdates, y=upper, line=dict(width=0), showlegend=False, hoverinfo="skip"))
        fig.add_trace(
            go.Scatter(
                x=fdates,
                y=lower,
    ...
    fig.update_layout(height=520, xaxis_rangeslider_visible=False, **theme.PLOTLY_LAYOUT)
    st.plotly_chart(fig, width="stretch")
    st.caption("Shaded band = ±1.28σ (≈80%) random-walk uncertainty around the central forecast.")

with tab_cmp:
    st.markdown("#### Held-out walk-forward performance")
    st.caption(f"Test period: {cutoff.date()} → present · pooled across all {len(tickers)} tickers")
    metrics = meta.get("metrics", {})
    if metrics:
        table = pd.DataFrame(metrics).T.rename(
            columns={
        ...
        bar = go.Figure(
            go.Bar(
                x=order,
                y=[metrics[m]["directional_accuracy"] * 100 for m in order],
                marker_color=["#1f77b4" if m == "xgboost" else "#b0bec5" for m in order],
                text=[f"{metrics[m]['directional_accuracy'] * 100:.1f}%" for m in order],
                textposition="outside",
            )
        )
        bar.add_hline(y=50, line_dash="dot", annotation_text="coin-flip (50%)")
        bar.update_layout(height=360, title="Directional accuracy (%)", **theme.PLOTLY_LAYOUT)
        st.plotly_chart(bar, width="stretch")
        st.info(
            "On daily **price RMSE** the random walk is hard to beat — the correct, leakage-free result. "
            "The edge shows in **directional accuracy**, where tuned XGBoost clears 50% and beats ARIMA."
        )

with tab_bt:
    st.markdown("#### Out-of-sample strategy backtest")
    st.caption(
        "Hold the stock when the model predicts a positive next-day return, else cash. Out-of-sample only; "
        "ignores costs/slippage — **illustrative, not financial advice.**"
    )
    ...
    if len(feats) < 20:
        ...
        strat = (preds > 0).astype(int) * feats["target"].to_numpy()
        bh = feats["target"].to_numpy()
        cum_strat, cum_bh = np.exp(np.cumsum(strat)), np.exp(np.cumsum(bh))
        ...
        k4.metric("Strategy Sharpe", f"{np.mean(strat) / (np.std(strat) + 1e-9) * np.sqrt(252):.2f}")
        bt = go.Figure()
        bt.add_trace(go.Scatter(x=feats.index, y=cum_strat, name="Model strategy", line=dict(color="#1f77b4", width=2)))
        bt.add_trace(
            go.Scatter(x=feats.index, y=cum_bh, name="Buy & hold", line=dict(color="#9aa6b2", width=2, dash="dash"))
        )
        bt.update_layout(height=420, title=f"Growth of $1 — {ticker}", **theme.PLOTLY_LAYOUT)
